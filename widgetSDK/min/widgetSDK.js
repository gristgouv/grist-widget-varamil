...
<div class="switch_slider"></div><div class="switch_circle"></div></div>`;
      ...
      case "longstring":
        return `<textarea ${r}class="config-textarea auto-expand ${t.code ? "code" : ""}" oninput="this.style.height = ''; this.style.height = this.scrollHeight + 'px'" ${this.#t(t.event)}>${l}</textarea>`;
      case "object":
      ...
      case "dropdown":
        ...
        return s[t.id] && s[t.id].forEach((c) => {
          o += `<option value="${this.escapeQuotes(c)}" ${c === l ? "selected" : ""}>${c}</option>`;
        }), o + "</select>";
      case "button":
        return `<button id="action-button" class="config-button" ${this.#t(t.event)}>${this.t(t.label ?? "Execute")}</button>`;
      default:
        return `<input ${r}class="config-input ${t.code ? "code" : ""}" value="${this.escapeQuotes(l)}" ${this.#t(t.event)}>`;
    }
  }
  #t(t) {
    if (!t) return "";
      ...
      case "templateform":
        ...
        return s.forEach((n, l) => {
          ...
          Object.keys(n).forEach((o) => {
            let c = e.querySelector("#" + e.id + "_" + l + "_" + o);
            c ? r[o] = this.#i(t.template.find((h) => h.id === o), c) : r[o] = void 0;
          }), a.push(r);
        }), a;
      default:
